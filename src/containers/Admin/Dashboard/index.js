...
const latestOrdersHead = ['#', 'booking id', 'user', 'date', 'mode of payment', 'total price'];
...
const Dashboard = () => {
	...
	const totalRevenue = statistic.totalRevenue ? numberWithCommas(statistic.totalRevenue) : 0;
	...
	let dayData = [];
	const map_booking_day = statistic.map_booking_day ? statistic.map_booking_day : [];
										...
										<tbody>
											{topRoomData.map((item, index) => (
												<tr key={index}>
													...
													<td>{item.room}</td>
													<td>
														<Badge className='badge-status' pill bg={generalStatus[item.type]}>
															{item.type}
														</Badge>
													</td>
