...
const InvoiceRevenue = () => {
	...
	const headers = [
		...
		{ name: 'Deposit (USD)', field: 'deposit', sortable: false },
		...
		{ name: 'Total Price (USD)', field: 'totalPrice', sortable: false },
		{ name: 'Paid Out (USD)', field: 'paidOut', sortable: false },
		{ name: 'Refund (USD)', field: 'refund', sortable: false },
	];
	...
	const currentData = useMemo(() => {
		let computedReceipts = statistic.invoiceRevenue
			? statistic.invoiceRevenue
					...
					.filter(
						(item) =>
							new Date(item.checkOutDate) < endDate && new Date(item.checkOutDate) > startDate
					)
			: [];
	...
	}, [statistic.invoiceRevenue, currentPage, limit, sorting, endDate, startDate]);
