...
export const updateUser = (updateUser, id) => {
	return async (dispatch) => {
		try {
			dispatch({ type: types.SET_USER_LOADING, payload: true });
			const response = await axios.put(`${HOST_API_URL}/user/update/${id}`, updateUser);
			if (response.data.success) {
				dispatch({
...
export const updateProfile = (updateUser, id) => {
	return async (dispatch) => {
		try {
			...
			const response = await axios.put(`${HOST_API_URL}/user/update-profile/${id}`, updateUser);
			if (response.data.success) {
				dispatch({
					type: types.UPDATE_USER,
					payload: response.data.updatedUser,
...
export const changePassword = (updateUser, id) => {
	return async (dispatch) => {
		try {
			dispatch({ type: types.SET_USER_LOADING, payload: true });
			const response = await axios.put(`${HOST_API_URL}/user/change-password/${id}`, updateUser);
			if (response.data.success) {
				dispatch({
					type: types.UPDATE_USER,
					payload: response.data.updatedUser,
